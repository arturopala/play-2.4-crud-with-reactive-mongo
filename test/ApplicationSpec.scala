...
class ApplicationSpec extends WordSpecLike with Matchers with PropertyChecks with CommonGenerators with ScalaFutures {
  ...
  "Application" should {
    ...
    "provide an API to create, find, update and delete vessel" in new WithApplication {
      forAll(VesselGenerator) { (vessel: Vessel) =>
        ...
        val criteria = Json.stringify(Json.toJson(vessel))
        ...
        status(updateResponse2) should be(BAD_REQUEST)
      }
    }
